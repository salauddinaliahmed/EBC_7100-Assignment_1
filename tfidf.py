...
for each_list in book1_words:
        clean_book1_words.append(([set(each_word for each_word in each_list if not each_word in stop_words if each_word.isalpha())], 'KJV (Bible)'))
print (clean_book1_words[0])

#clean book 2
clean_book2_words = []
for each_list in book2_words:
        clean_book2_words.append(([set(each_word for each_word in each_list if not each_word in stop_words if each_word.isalpha())], 'Herman Melville(Moby-Dick)'))
        vectorizer = TfidfVectorizer()
        X = vectorizer.fit_transform(each_list)
        feature_names = vectorizer.get_feature_names()
        corpus_index = [n for n in each_list]
